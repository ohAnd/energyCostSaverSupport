...
void DisplayTFT::drawScreen(String version, String time)
{
    ...
    lastDisplayData.rssiGW = 0; //platformData.wifi_rssi_gateway;

    // reset screen after certain state transitions
    if (lastDisplayData.stateWasOffline || lastDisplayData.stateWasCloudPause || lastDisplayData.stateWasNormal)
    {
        tft.fillScreen(TFT_BLACK);
...
{
    // save display value
    // lastDisplayData.totalPower = round(espGlobalData.grid.power);
    // lastDisplayData.powerLimit = espGlobalData.powerLimit;
...
{
    valueChanged = false;
    // if (lastDisplayData.totalPower != round(espGlobalData.grid.power))
    //     valueChanged = true;
}
